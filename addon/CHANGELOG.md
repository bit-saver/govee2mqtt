...
## [2026.05.27-943a9f3c] - 2026-05-28 00:40

### ⚙️ Miscellaneous

- Update addon version [skip ci]
- Gitignore update

### 🐛 Bug Fixes

- Fix: remove IoT fallback from scene-code (wrong type + same ptReal limitation)

### 🚀 Features

- Feat: add POST /api/device/{id}/scene-code/{code} endpoint

Accepts an arbitrary effectStr (as scence_param) + scene_code and
sends it to the device using the same SetSceneCode encoding that
preset scenes use. Tries LAN API first, falls back to IoT.

This enables GIF-based DIY playback from Otto's library without
needing Govee's big-command URL — the device receives the chunked
frames directly via LAN or IoT, bypassing the ptUrl firmware
domain whitelist that blocks non-Govee URLs.
- Feat: add gv2mqtt/scene-code MQTT handler for GIF DIY playback

Subscribes to topic `gv2mqtt/scene-code` and accepts a JSON payload
with device ID, scene_code, and scence_param (base64 effectStr).
Routes through the existing SetSceneCode LAN path — same encoding
that preset scenes use, bypassing ptUrl domain whitelisting.

Otto publishes to this topic for GIF-based DIYs instead of trying
ptReal (device rejects 1000+ inline frames) or ptUrl (firmware
whitelists app.govee.com).

Payload: { "device": "<id>", "sceneCode": 255, "scenceParam": "..." }

### 🧹 Refactor

- Refactor: rename scene-code → diy-code for MQTT topic + HTTP endpoint

## [2026.04.14] - 2026-04-15 01:00

### ⚙️ Miscellaneous

- Ci: bump deprecated Node.js 20 actions to v4/v6
...
- Update addon version [skip ci]
- Merge branch 'main' of github.com:bit-saver/govee2mqtt

### 🐛 Bug Fixes

- Fix: replace slash with hyphen in MQTT client ID

The forward slash in the client ID format "govee2mqtt/{uuid}" was
triggering Mosquitto's ACL security in the HA addon, which treats
slashes in client IDs as dangerous (potential topic traversal).
This caused all MQTT publish/subscribe operations to be silently
denied, breaking all HA device control while LAN API and HTTP
continued working.

## [2026.03.25.1] - 2026-03-25 18:17
...
- Add H60A1 quirk with correct color temperature range (2200K-6500K) ([#502](https://github.com/bit-saver/govee2mqtt/issues/502))
...
## [2025.11.05-c34d0190] - 2025-11-05 07:32

### ⚙️ Miscellaneous

- Remove hassio_api bit

## [2025.11.05-ca05ba52] - 2025-11-05 07:17
...
## [2026.03.25] - 2026-03-25 18:07

### ⚙️ Miscellaneous

- Streamline CI: single workflow run per deploy

- Remove branch push trigger, keep only tag pushes and PRs
- Have addon job push config.yaml version back to repo
- Update Docker meta to apply latest tag on tag pushes
- Simplify test-addon condition to PR-only
...
Govee's login endpoint now enforces a minimum app version check.
Bumps APP_VERSION from 5.6.01 to 6.5.02 and adds the required
headers to login_account_impl(), matching the pattern already
used in get_iot_key() and get_device_list().

Based on wez/govee2mqtt#625

Co-Authored-By: Claude Opus 4.6 (1M context) <noreply@anthropic.com>

## [2026.01.27] - 2026-01-27 18:43

### 🐛 Bug Fixes

- Fix mqtt_diys deserialization: add serde defaults for action_type and cmd_group

These fields were required but missing from the JSON payloads sent via
the mqtt_diys topic, causing deserialization to fail with "missing field"
errors.

Co-Authored-By: Claude Opus 4.5 <noreply@anthropic.com>

## [2026.01.26] - 2026-01-27 04:12

### ⚙️ Miscellaneous

- Merge from wez/govee2mqtt
- Quick fix after merge

## [2024.12.20-08616337] - 2024-12-20 16:00

### ⚙️ Miscellaneous

- Oneclick debugging
- Updated oneclick schema

## [2024.09.29-fa23f383] - 2024-09-30 00:40
